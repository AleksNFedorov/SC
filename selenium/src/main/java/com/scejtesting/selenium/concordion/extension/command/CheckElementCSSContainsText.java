 ...
 */
public class CheckElementCSSContainsText extends AbstractCheckElementContentCommand {

    public CheckElementCSSContainsText(AssertListener listener) {
        super(listener);
    }
    ...
    protected boolean doCheck(By predicate, String content, List allParameters) {
        ...
        return getTestFixture().checkElementCSSContainsText(predicate, content, textToSearch);
    }

    @Override
    protected boolean doCheck(WebElement element, String content, List allParameters) {
        String textToSearch = allParameters.get(2).toString();
        return getTestFixture().checkElementCSSContainsText(element, content, textToSearch);
    }

    @Override
    public String getCommandName() {
        return "checkElementCSSContainsText";
    }
}
